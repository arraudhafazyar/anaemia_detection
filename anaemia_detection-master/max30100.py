...
class HeartRateMonitor:
    ...
    def calculate_bpm(self):
        ...
        threshold = np.mean(smoothed) + 0.3 * (np.max(smoothed) - np.min(smoothed))
        peaks = []
        ...
        if len(peaks) >= 2:
            ...
            for i in range(1, len(peaks)):
                ...
                if 0.4 < time_diff < 2.0:  # Filter unrealistic intervals (30-150 BPM)
            ...
            if peak_intervals:
                ...
                # Smooth BPM changes
                if 40 <= new_bpm <= 200:  # Valid heart rate range
                    if self.bpm == 0:
                        self.bpm = new_bpm
                    else:
